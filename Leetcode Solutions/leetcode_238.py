...
# Without using division operator I
def product_except_self(nums: list[int]) -> list[int]:
    ...
    results = [prefix[i] * suffix[i] for i in range(n)]
...
# Without using division operator II
def product_except_self_i(nums: list[int]) -> list[int]:
    n = len(nums)
    results = [1] * n
...
# Examples
print(product_except_self_i([1,2,3,4]))        # [24, 12, 8, 6]
print(product_except_self_i([-1,1,0,-3,3]))    # [0, 0, 9, 0, 0]
